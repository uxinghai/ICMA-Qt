...
//
//   // 日志文件
//   connect(ui->actionEnableFileLog, &QAction::triggered, this,
//           &MainWindow::doEnableLogOut);
//
//   // 关于QT
...
//
//   // 启动图像处理窗口
//   connect(ui->actionPS, &QAction::triggered, [this] {
...
//
//   // 启动文件传输
//   connect(ui->actionTransmission, &QAction::triggered, [this] {
...
//           [this](const QString& filePath, const QString& hashValue) {
//             if (filePath == "NULL") {
//               lbStatus->setText(tr("%1 个对象").arg(filesCountResult));
//             }
//             else {
//               lbStatus->setText(filePath);
//               // 依据哈希值推荐相关文件
//               doRecFileByHash(hashValue);
//             }
//           });
//   connect(ui->tableView, &MyTableView::updateRowCount,
//           [this](const qint32& rowCount) {
//             lbStatus->setText(tr("%1 个对象").arg(rowCount));
//           });
//
...
//     progress->setVisible(false);
//     // 线程完成后从数据库获取内容。线程开始前会写入数据库
//     filesCountResult = sFileDB.getAllFilesShowView(daoRuDirectoryFile,
//                                                     ui->tableView, 0);
//     lbStatus->setText(
//       tr("%1 个对象").arg(sFileDB.getFileCount(daoRuDirectoryFile)));
...
//   else if (objectName == "actionPreview") { ui->lbPreview->setVisible(checked); }
...
//
//   // 写回文件
//   QSettings setting = iniManager::getIniSetting();
//   setting.setValue("Settings/theme-style", qssPath);
// }
//
// void MainWindow::doSearchFile(const QString& term, const quint8& filterMode)
// {
//   //bool isRegexSearch = false;
//
//   // 检查 term
//   // if (!term.isEmpty()) {
//   //   // 检查是否以正则表达式标记开始（例如使用 "regex:" 前缀）
//   //   if (term.startsWith("regex:", Qt::CaseInsensitive)) {
//   //     isRegexSearch = true;
//   //     searchTerm = term.mid(6).trimmed(); // 去除"regex:"前缀
//   //
//   //     // 验证正则表达式是否有效
//   //     if (const QRegularExpression regex(searchTerm); !regex.isValid()) {
//   //       // 显示错误消息
//   //       QMessageBox::warning(this, tr("无效的正则表达式"),
//   //                            tr("您输入的正则表达式无效: %1").arg(regex.errorString()));
//   //       return;
//   //     }
//   //   }
//   // }
//
//   if (const QString searchTerm = term; searchTerm.isEmpty()) { ///< 搜索为空时显示所有文件
//     filesCountResult = sFileDB.getAllFilesShowView(
//       daoRuDirectoryFile, ui->tableView, filterMode);
//   }
//   else {
//     // 使用新的搜索方法，传递过滤模式
//     filesCountResult = sFileDB.searchFilesShowView(
//       searchTerm, daoRuDirectoryFile, ui->tableView, true, filterMode);
//   }
//
//   ui->tableView->setColumnWidth(0, 500); ///< 让名称列显示的更宽一些
//   ui->tableView->setTheSelectionModel(
...
//                                       ui->actionShowHashCol,
//                                       ui->actionShowEncrCol);
//   lbStatus->setText(tr("%1 个对象").arg(ui->tableView->model()->rowCount()));
// }
//
// void MainWindow::doRecFileByHash(const QString& hashValue)
// {
...
//   else if (ui->actionEN->isChecked()) { context = GetIcmaBrief::getIcmaBrief("EN"); }
//   else if (ui->actionJP->isChecked()) { context = GetIcmaBrief::getIcmaBrief("JP"); }
//
...
//
// // 写入数据库的映射函数
// qint32 MainWindow::doFilesFormDirectory(const QString& dirPath) const
// {
//   static QMutex mutex;
...
//   future = mappedReduced(tempListPath,
//                          [this](const QString& path) {
//                            return this->doFilesFormDirectory(path);
//                          },
...
// }
//
// void MainWindow::showFileContextMenu() const
// {
//   // 获取点击文件的类型显示不同的右键菜单
...
//   const auto menu = std::make_unique<QMenu>();
//   // 文件操作基本功能
//   menu->addAction(ui->actionFileOpen);
//   menu->addAction(ui->actionFileOpenExplore);
//   menu->addAction(ui->actionFileCopyName);
...
//
//   // 如果时图像则包含编辑图像功能菜单项
//   const QStringList PixmapSuffixes{
//     "png", "jpg", "jpeg", "bmp", "gif", "svg", "ico", "webp"
//   };
//   if (PixmapSuffixes.contains(
//     indexes.first().siblingAtColumn(3).data().toString())) {
//     menu->addAction(ui->actionImgEditorPix);
//   }
//
//   // 文本文件操作
...
//   };
//   if (TextSuffixes.contains(
//     indexes.first().siblingAtColumn(3).data().toString())) {
//     menu->addAction(ui->actionEditText);
//   }
//
//   menu->addSeparator();
//   menu->addAction(ui->actionFileCompress);
//
//   menu->addAction(ui->actionRefresh);
//   menu->exec(QCursor::pos());
// }
