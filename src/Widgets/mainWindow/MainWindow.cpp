...
#include "../../DataBase/SqlQuery/Tags.h"
    ...
    progress(new QProgressBar(this)), updateChecker(new UpdateChecker(this))
{
  ...
  ui->lbTag1->setVisible(false);
  ui->lbTag2->setVisible(false);
...
void MainWindow::setupConnections()
{
  ...
  connect(ui->tableView, &MyTableView::lbStatusModify,
          [this](const QString& fileAbsPath, const QString& hashValue) {
            if (fileAbsPath == "NULL") {
              lbStatus->setText(tr("%1 个对象").arg(filesCountResult));
            }
            else {
              lbStatus->setText(fileAbsPath);

              // 获取 Tags显示 并且显示文件信息
              const QStringList tags = sTagDB.getTagsByFileID(
                sFileDB.getFileIdByFileAbsPath(fileAbsPath, "TempFiles"));
              switch (tags.size()) {
              case 3:
                ui->lbTag1->setVisible(true);
                ui->lbTag2->setVisible(true);
                ui->lbTag3->setVisible(true);
                break;
              case 2:
                ui->lbTag1->setVisible(true);
                ui->lbTag2->setVisible(true);
              ...
              const QFileInfo info(fileAbsPath);
              ui->lbFileInfoFileName->setText(info.fileName());
              ui->lbFileInfoFileHash->setText(hashValue);
              ui->lbFileInfoFileModTime->setText(
                info.birthTime().toString("yyyy/MM/dd hh:mm"));

              ui->lbFileInfoFileOpenCount->setText(
                QString::number(sFileActionsDB.getFileOpenCntByAbsPath(fileAbsPath)));
